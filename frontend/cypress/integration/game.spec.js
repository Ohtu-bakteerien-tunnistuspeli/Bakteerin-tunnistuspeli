describe('Playing game', () => {
    ...
    describe('Game can be played', () => {
        ...
        it('After choosing right samplingmethod, user can choose all tests in required order', () => {
            ...
            cy.wait(500)
            cy.contains('CAMP-testi').click({timeout: 1000, waitForAnimations: true})
            // FIX THIS AFTER MORE INFORMATIVE MESSAGES ARE SET
            cy.contains('Väärä vastaus')
            cy.contains('Testejä').click()
            cy.contains('Veriagar, +37 C, aerobinen kasvatus').click({force: true})
            cy.contains('Oikea vastaus')
            cy.contains('Testejä').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Gramvärjäys').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Katalaasitesti').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('HIRS-sarja').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Eskuliiniveriagar').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Edwardsin agar').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Lancefield määritys').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Penisilliinin sietokoe agarvaluamenetelmällä').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('CAMP-testi').click({timeout: 1000, waitForAnimations: true})
        ...
        it('After choosing right samplingmethod, user can choose only required tests in required order', () => {
            ...
            cy.get('[type="checkbox"]').eq('2').check()
            cy.get('#checkSamples').click()
            cy.contains('Veriagar, +37 C, aerobinen kasvatus', { timeout: 5000, waitForAnimations: true }).click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Gramvärjäys').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Testejä').click()
            cy.contains('Katalaasitesti').click({timeout: 1000, waitForAnimations: true})
            ...
            cy.contains('Testejä').click()
            cy.contains('Penisilliinin sietokoe agarvaluamenetelmällä').click({timeout: 1000, waitForAnimations: true})
            cy.contains('Oikea vastaus. Kaikki vaaditut testit tehty.', { timeout: 5000, waitForAnimations: true })
        })
